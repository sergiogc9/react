...
const getElementScrollData = (element: HTMLElement): ScrollData => {
	const hasScroll = element.scrollWidth - element.clientWidth > 0;
	const px = element.scrollLeft;
	const percentage = hasScroll ? (element.scrollLeft / (element.scrollWidth - element.clientWidth)) * 100 : 0;
 ...
 */
const useIsHorizontalScrolled = (ref: React.RefObject<HTMLElement>) => {
...
export default useIsHorizontalScrolled;
